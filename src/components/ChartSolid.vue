<template>
  <div class="chart-container" data-cy="chart-data-cell">
    <canvas ref="canvas" width="450" height="70">Solid</canvas>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    ctx.strokeStyle = frameworks[8].color;
    ctx.lineWidth = 2;
    ctx.stroke();

    // lägg till texten "Solid" före linjen
    const textBeforeLineX = startX - 30; // justera X-koordinaten för texten
    ctx.fillStyle = frameworks[8].color; // färgen på texten
    ctx.font = 'bold 11px Arial'; // stil och storlek på texten
    ctx.textAlign = 'right'; // textens justering
    ctx.textBaseline = 'middle';
    ctx.fillText('Solid', textBeforeLineX, startY);
    ...
    //for (let i = 0; i < numRings; i++) {
      ...
      for (let i = 0; i < numRings; i++) {
        ...
        const x = startX + ((year + 5) / (6 - 0)) * (endX - startX);
        const text = this.ringTexts[i];
