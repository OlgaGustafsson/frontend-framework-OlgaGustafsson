<template>
  <div class="chart-container" data-cy="chart-data-cell">
    <canvas ref="canvas" width="450" height="70">Stencil</canvas>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    ctx.strokeStyle = frameworks[10].color;
    ctx.lineWidth = 2;
    ctx.stroke();

    // lägg till texten "Stencil" före linjen
    const textBeforeLineX = startX - 30; // justera X-koordinaten för texten
    ctx.fillStyle = frameworks[10].color; // färgen på texten
    ctx.font = 'bold 11px Arial'; // stil och storlek på texten
    ctx.textAlign = 'right'; // textens justering
    ctx.textBaseline = 'middle';
    ctx.fillText('Stencil', textBeforeLineX, startY);
    ...
    // lägg till texten "Stencil" efter linjen
    const textAfterLineX = lineAfterLastRingX + 40; // justera X-koordinaten för texten
    ctx.fillText('Stencil', textAfterLineX, endY);
    ...
    //for (let i = 0; i < numRings; i++) {
      ...
      for (let i = 0; i < numRings; i++) {
        ...
        const x = startX + ((year + 6) / (6 - 0)) * (endX - startX);
        const text = this.ringTexts[i];
