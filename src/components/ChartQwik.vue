<template>
  <div class="chart-container" data-cy="chart-data-cell">
    <canvas ref="canvas" width="450" height="70">Qwik</canvas>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    ctx.strokeStyle = frameworks[9].color;
    ctx.lineWidth = 2;
    ctx.stroke();

    // lägg till texten "Qwik" före linjen
    const textBeforeLineX = startX - 30; // justera X-koordinaten för texten
    ctx.fillStyle = frameworks[9].color; // färgen på texten
    ctx.font = 'bold 11px Arial'; // stil och storlek på texten
    ctx.textAlign = 'right'; // textens justering
    ctx.textBaseline = 'middle';
    ctx.fillText('Qwik', textBeforeLineX, startY);
