<template>
  <div class="chart-container" data-cy="chart-data-cell">
    <canvas ref="canvas" width="450" height="70">Ember</canvas>
  </div>
</template>
...
export default {
    data() {
        return {
            ringTexts: ['47%', '41%', '45%', '31%', '27%', '21%', '17%'],
    };
},
  ...
  mounted() {
    ...
    ctx.strokeStyle = frameworks[3].color;
    ctx.lineWidth = 2;
    ctx.stroke();

    // lägg till texten "Ember" före linjen
    const textBeforeLineX = startX - 30; // justera X-koordinaten för texten
    ctx.fillStyle = frameworks[3].color; // färgen på texten
    ctx.font = 'bold 11px Arial'; // stil och storlek på texten
    ctx.textAlign = 'right'; // textens justering
    ctx.textBaseline = 'middle';
    ctx.fillText('Ember', textBeforeLineX, startY);
