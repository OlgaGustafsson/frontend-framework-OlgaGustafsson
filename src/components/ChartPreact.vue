<template>
  <div class="chart-container" data-cy="chart-data-cell">
    <canvas ref="canvas" width="450" height="70">Preact</canvas>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ...
    ctx.strokeStyle = frameworks[4].color;
    ctx.lineWidth = 2;
    ctx.stroke();

    // lägg till texten "Preact" före linjen
    const textBeforeLineX = startX - 30; // justera X-koordinaten för texten
    ctx.fillStyle = frameworks[4].color; // färgen på texten
    ctx.font = 'bold 11px Arial'; // stil och storlek på texten
    ctx.textAlign = 'right'; // textens justering
    ctx.textBaseline = 'middle';
    ctx.fillText('Preact', textBeforeLineX, startY);
    ...
    // lägg till texten "Preact" efter linjen
    const textAfterLineX = lineAfterLastRingX + 40; // justera X-koordinaten för texten
    ctx.fillText('Preact', textAfterLineX, endY);
    ...
    for (let i = 0; i < numRings; i++) {
        ...
        const x = startX + ((year + 2) / (6 - 0)) * (endX - startX);
        const text = this.ringTexts[i];
